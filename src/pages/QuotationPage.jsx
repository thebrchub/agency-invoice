...
import {
  ...
  Award, Clock, ChevronRight, Palette
} from 'lucide-react';

/* --- CONSTANTS --- */
const STORAGE_KEY = 'quotation_builder_v11_custom_color'; // Updated version key
...
// Legal snippets to solve the "Ownership & Warranty" critique
...
const DEFAULT_DATA = {
  ...
  projectTitle: '',
  projectDescription: '',
  projectGoals: '', // "Business Outcome"
  companyHighlights: '', // "Why Us"
  timelineTitle: 'Estimated Timeline', // Customizable Heading
  ...
  showSignatory: true,
  signatoryName: '',
  notes: '', // Empty by default, populated via snippets
  logo: null,
  signature: null,
  qrCode: null,
...
const RichTextRenderer = ({ text, className }) => {
  if (!text) return null;
  // Added break-words and whitespace-pre-wrap to prevent overflow
  return (
...
const QuotationGenerator = () => {
  const [data, setData] = useState(() => {
    try {
      ...
      // Migration: Map old themes to hex colors if brandColor is missing
      let initialColor = DEFAULT_DATA.brandColor;
      if (parsed.selectedTheme && !parsed.brandColor) {
         if (parsed.selectedTheme === 'minimal') initialColor = '#1e293b'; // slate-800
         else if (parsed.selectedTheme === 'emerald') initialColor = '#047857'; // emerald-700
         else if (parsed.selectedTheme === 'rose') initialColor = '#be123c'; // rose-700
         else initialColor = '#4338ca'; // indigo-700 (classic)
      }
  ...
  // Helper for light/dark text on custom background
  // Simple check: if color is dark, return white text, else black
  // For simplicity, we'll assume most brand colors are dark/saturated enough for white text in headers
  ...
  return (
    <div className={`min-h-screen font-sans selection:bg-indigo-500/30 ${isDark ? 'bg-slate-950 text-slate-100' : 'bg-slate-50 text-slate-900'}`}>
      ...
      {/* FIXED SIDEBAR */}
      <div className={`fixed top-0 left-0 w-[480px] h-screen flex flex-col border-r z-50 no-print ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-gray-200'}`}>
        <div className="px-6 py-4 border-b flex justify-between items-center shrink-0 z-10 bg-inherit">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h2 className="text-lg font-bold flex items-center gap-2"><Briefcase size={18} className="text-indigo-500"/> Quotation</h2>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 scrollbar-thin">
          ...
          {/* 3. STRATEGY & SCHEDULE (NEW - Addresses Critique 1, 2, 5, 6) */}
          <div className={sectionClass}>
              ...
              <div className="mt-5 space-y-4 animate-fade-in-down">
                {data.items.map((item) => (
                  <div key={item.id} className={`relative p-3 rounded-lg border group transition-all ${isDark ? 'bg-slate-800/50 border-slate-700' : 'bg-slate-50 border-slate-200'}`}>
                    <button onClick={() => removeItem(item.id)} className="absolute -top-2 -right-2 bg-red-100 text-red-500 rounded-full p-1 opacity-0 group-hover:opacity-100 transition-opacity shadow-sm"><Trash2 size={12}/></button>
          ...
          {/* 5. LEGAL (Enhanced with Snippets) */}
          <div className={sectionClass}>
            ...
            {openSections.payment && (
              <div className="mt-5 space-y-4 animate-fade-in-down">
                ...
                <div className="pt-2">
                   ...
                   {/* Quick Insert Buttons for Critique 3 & 4 */}
      ...
      {/* RIGHT PANEL (PREVIEW) */}
      <div className="flex-1 bg-gray-100 overflow-y-auto p-4 md:p-10 print-container md:ml-[480px]">
        ...
        <div className="hidden print:block fixed bottom-0 left-0 w-full text-center pb-4 bg-white z-50">
          <p className="text-[10px] text-slate-400 font-medium">Generated by BRC Hub LLP</p>
        </div>

        {/* A4 DOCUMENT PAGE */}
        <div className="bg-white w-full max-w-[210mm] min-h-[297mm] mx-auto p-12 shadow-2xl relative text-slate-900 print:shadow-none print:w-full print:max-w-none flex flex-col page-break-inside-avoid">
          {/* Header */}
          ...
          {/* NEW: STRATEGIC SECTION (Objectives & Why Us) - Addressing Critique 1 & 2 */}
          ...
          {/* NEW: TIMELINE - Addressing Critique 6 */}
          ...
          {/* QR CODE - ONLY ON FIRST PAGE IF POSSIBLE, OR ABSOLUTE */}
        ...
        {data.annexure?.show && (
           <div className="bg-white w-full max-w-[210mm] min-h-[297mm] mx-auto p-12 shadow-2xl relative text-slate-900 mt-10 page-break print:mt-0 print:min-h-0 print:h-auto">
